...
class Article extends React.Component {
  ...
  render() {
    ...
    const articleBody = body.map(article => { return ReactHtmlParser(article.data) })

    return (
      <div className="article">
        <h1 className="title" onClick={() => this.toggle()}>{title}</h1>
        <Transition
          ...
          {isOpen =>
            isOpen && (props => (
              <div style={props} className="content">
                <img className="articleImg" src={img} alt={alt} />{articleBody}
              </div>
            )
            )
          }
